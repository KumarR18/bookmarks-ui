import { B as BookMarkActionTypes, c as createCommonjsModule, b as commonjsGlobal, d as browser$1 } from './index-35b42073.js';

const downloadBookmarks = () => ({
  type: BookMarkActionTypes.BEGIN_BOOKMARKS_DOWNLOAD,
  payload: true,
});
const downloadBookmarksOk = (bookmarks) => ({
...
const downloadBookmarksAction = () => async (dispatch) => dispatch(downloadBookmarks());
const downloadBookmarksOkAction = (bookmarks) => async (dispatch) => dispatch(downloadBookmarksOk(bookmarks));
const downloadBookmarksErrorAction = () => async (dispatch) => dispatch(downloadBookmarksError());
const addBookmark$1 = (bookmark) => ({
...
const addBookmarkError = (state) => ({
  type: BookMarkActionTypes.ADD_BOOKMARK_ERROR,
  payload: state,
});
const addBookmarkAction = (bookmark) => async (dispatch) => dispatch(addBookmark$1(bookmark));
const addBookmarkOkAction = () => async (dispatch) => dispatch(addBookmarkOk());
const addBookmarkErrorAction = (state) => async (dispatch) => dispatch(addBookmarkError(state));
const retrieveBookmarkDelete = (bookmark) => ({
...
const deleteBookmarkAction = (bookmark) => async (dispatch) => dispatch(retrieveBookmarkDelete(bookmark));
const deleteBookmarkOkAction = () => async (dispatch) => dispatch(deleteBookmarkOk());
const deleteBookmarkErrorAction = () => async (dispatch) => dispatch(deleteBookmarkError());
...
const displayPopup = (icon, title, text, showCancelButton) => {
  const showMessage = {
    title,
    text,
    icon,
  ...
  const confirmation = Object.assign(Object.assign({}, showMessage), { showCancelButton, cancelButtonColor: '#f66b61', confirmButtonText: 'Yes, delete it!' });
  return sweetalert2_all.fire(showCancelButton ? confirmation : showMessage);
};
...
async function retrieveBookmarksDB() {
  return await axiosClient.get('/bookmarks');
}
async function addBookmarkDB(bookmarks) {
  return await axiosClient.post('/bookmarks', bookmarks);
}
async function deleteBookmarksDB(bookmark) {
...
export { displayPopup as a, addBookmarkAction as b, addBookmark as c, displayToastMessage as d, deleteBookmarkAction as e, deleteBookmark as f, retrieveBookmarks as r };
